...
- Quality: {'The Guardian': 0.9, 'Wikipedia': 0.8, 'Global Wire': 0.85} (+ 0.50 for unknown sources); diversity band ±0.05.
...
| P8    | diversity alternates source types within the ±0.05 band                                          | PASS | PASS | PASS | PASS | PASS |
...
- Corpus caveat: the v2 corpus timestamps correlate with relevance by authoring (M3-C, rho ≈ 0.41), so adding freshness can inflate corpus nDCG without meaning it is better: the probes are the controlled evidence, corpus numbers are indicative.
